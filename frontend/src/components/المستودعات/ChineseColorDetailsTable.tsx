...
interface ChineseColorDetailsTableProps {
  type: string;
  color: string;
  onBack: () => void;
}

const ChineseColorDetailsTable: React.FC<ChineseColorDetailsTableProps> = ({ 
  type, 
  color, 
  onBack 
}) => {
  const [details, setDetails] = useState<ChineseColorDetail[]>([]);
  const [filteredDetails, setFilteredDetails] = useState<ChineseColorDetail[]>([]);
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/warehouse/chinese/color-details/${encodeURIComponent(type)}/${encodeURIComponent(color)}`);
  ...
  const totalLength = filteredDetails.reduce((sum, detail) => sum + detail.long, 0);
                  ...
                  filteredDetails.map((detail, index) => (
                    <tr key={index} className="hover:bg-orange-50 transition-colors duration-200">
                      <td className="px-6 py-5 whitespace-nowrap text-center">
                        <div className="bg-gradient-to-r from-purple-500 to-purple-600 text-white px-3 py-1 rounded-md shadow-sm inline-block">
                      ...
                      <td className="px-6 py-5 whitespace-nowrap text-center">
                        <div className="text-lg font-bold text-blue-600">
                          {detail.long.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </div>
...
export default ChineseColorDetailsTable;
