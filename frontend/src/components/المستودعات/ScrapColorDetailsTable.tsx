...
interface ScrapColorDetail {
  number: string;
  long2: number;
}

interface ScrapColorDetailsTableProps {
  desan: string;
  color: string;
  onBack: () => void;
}

const ScrapColorDetailsTable: React.FC<ScrapColorDetailsTableProps> = ({
  desan,
  color,
  onBack
}) => {
  const [details, setDetails] = useState<ScrapColorDetail[]>([]);
  const [filteredDetails, setFilteredDetails] = useState<ScrapColorDetail[]>([]);
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/warehouse/scrap/color-details/${encodeURIComponent(desan)}/${encodeURIComponent(color)}`);
  ...
  const totalLength = filteredDetails.reduce((sum, detail) => sum + detail.long2, 0);
...
export default ScrapColorDetailsTable;
