...
interface ScrapWarehouseDetail {
  desan: string;
  color: string;
  desan_count: number;
  total_long: string;
...
interface ScrapWarehouseDetailsTableProps {
  desan: string;
  onBack: () => void;
  onColorClick?: (desan: string, color: string) => void;
}

const ScrapWarehouseDetailsTable: React.FC<ScrapWarehouseDetailsTableProps> = ({
  desan,
  onBack,
  onColorClick
}) => {
  const [details, setDetails] = useState<ScrapWarehouseDetail[]>([]);
  const [filteredDetails, setFilteredDetails] = useState<ScrapWarehouseDetail[]>([]);
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/warehouse/scrap/details/${encodeURIComponent(desan)}`);
...
export default ScrapWarehouseDetailsTable;
