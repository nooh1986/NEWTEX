...
interface ClassicWarehouseDetail {
  desan: string;
  color: string;
  desan_count: number;
  total_long: string;
...
interface ClassicWarehouseDetailsTableProps {
  desan: string;
  onBack: () => void;
  onColorClick?: (desan: string, color: string) => void;
}

const ClassicWarehouseDetailsTable: React.FC<ClassicWarehouseDetailsTableProps> = ({ 
  desan, 
  onBack,
  onColorClick 
}) => {
  const [details, setDetails] = useState<ClassicWarehouseDetail[]>([]);
  const [filteredDetails, setFilteredDetails] = useState<ClassicWarehouseDetail[]>([]);
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        ...
        const endpoint = `http://localhost:5000/api/warehouse/classic/details/${encodeURIComponent(desan)}`;
  ...
  const handleColorClick = (color: string) => onColorClick?.(desan, color);

  // Calculate summary statistics
  const totalColors = filteredDetails.length;
  const totalAtwab = filteredDetails.reduce((sum, d) => sum + d.desan_count, 0);
  const totalAmtar = filteredDetails.reduce((sum, d) => {
    const cleanedValue = d.total_long?.replace(/,/g, '') || '0';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 text-center">
          ...
          >
            ...
            العودة لمستودع الكلاسيك
          </button>
          <h1 className="text-3xl font-bold text-gray-900">تفاصيل الدسان: {desan}</h1>
        </div>        {/* Search */}
        <div className="mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-xl shadow-lg border border-indigo-200 p-6 text-center transform hover:scale-105 transition-transform duration-200">
                    ...
                    >
                      ...
                      <td className="px-6 py-5 whitespace-nowrap text-center">
                        <div className="text-lg font-bold text-green-600">
                          {detail.desan_count.toLocaleString('en-US')}
                        </div>
...
export default ClassicWarehouseDetailsTable;
